  ...
  it('', function(done) {
    ...
    testApp.listen(8001, () => {
      ...
      .then((response) => {
        ...
        expect(appPutMatch, 'Did you remove extra PUT /animals routes from app.js?').not.be.ok;
        const appGetMatch = code.match(/app\.\s*get/);
        expect(appGetMatch, 'Did you remove extra /GET animals routes from app.js?').not.be.ok;
        const appPostMatch = code.match(/app\.\s*post/);
        expect(appPostMatch, 'Did you remove extra POST /animals routes from app.js?').not.be.ok;
        const appDeleteMatch = code.match(/app\.\s*delete/);
        expect(appDeleteMatch, 'Did you remove extra DELETE /animals routes from app.js?').not.be.ok;
        done();
      })
      .catch(done);
    });
